...
class BoxIterator:
    ...
    def __init__(self, *, denominator_localization : int = 2,
        ...
        if transformation == BoxTransformation.NOTRANSFORMATION:
            for (left_pt,right_pt) in bounding_box:
                if abs(left_pt+1.0)>eps or abs(right_pt-1.0)>eps:
            ...
            for axis_num,(left_bdry,right_bdry) in enumerate(bounding_box):
                ...
                if abs(seen_left_endpoint - left_bdry)>eps or \
                    abs(seen_right_endpoint - right_bdry)>eps:
                    #pylint:disable=line-too-long
                    raise ValueError(
                        f"The transformation should take -1,1 to {left_bdry},{right_bdry} or vice-versa")
